...
class FleetManager(Node):
    def __init__(self, config, nav_path):
        ...
        async def navigate(robot_name: str, cmd_id: int, dest: Request):
            ...
            goal_msg.pose.header.frame_id = 'map'  # or the appropriate frame
            ...
            if not goal_handle.accepted:
                response['msg'] = 'Goal rejected by Nav2 action server'
                return response

            # Wait for the server to complete the goal
            result_future = goal_handle.get_result_async()
            rclpy.spin_until_future_complete(robot.node, result_future)

            result = result_future.result().result
            if result.status == GoalStatus.STATUS_SUCCEEDED:
                response['success'] = True
            else:
                response['msg'] = 'Navigation failed'
